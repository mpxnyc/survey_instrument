...
export function getAvailableQuestions(surveyData, questionCurrent, questionHistory, questionList) {
    ...
    console.log("available question inputs", surveyData, questionCurrent, questionHistory, questionList)
    ...
    const result = questionList
        ...
        .filter((item) => {
            ...
            const lastQuestionIndex = questionList.findIndex((innerItem) => {return innerItem === surveyData[config.systemGeneratedVariables.variableNameForLastQuestion]})
            const skipLogicIndex    = questionnaire[item].skipLogic && questionList.findIndex((innerItem) => {return innerItem === questionnaire[item].skipLogic.question})
        ...
        .filter((item) => {
            ...
            (item) => {
                ...
                const questionIndex     = questionList.findIndex((innerItem) => {return innerItem === item} )
                const currentQuestionIndex = questionList.findIndex((innerItem) => {return innerItem === questionCurrent})
