...
export function getAvailableQuestions(surveyData, questionCurrent, questionHistory, questionList) {
    ...
    const result = questionList
        ...
        .filter((item) => {
            ...
            const skipLogicIndex    = questionnaire.ordering.findIndex((innerItem) => {return innerItem === questionnaire[item].skipLogic.question})

            const futureQuestion = questionIndex >= lastQuestionIndex
            ...
            const result = allowedSkip && futureQuestion || (surveyData[questionnaire[item].skipLogic.question] === "" && skipLogicIndex < lastQuestionIndex)
