...
const cleanDataSet = (dataSet) => {
    ...
    const radioQuestions = variableNames.filter(
        (item) => {
            ...
            return questionnaire[item] && (questionnaire[item].questionType === "radio" || questionnaire[item].questionType === "plaintext" || questionnaire[item].questionType === "radio_with_username_display");
        }
    )
