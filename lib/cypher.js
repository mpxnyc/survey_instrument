...
export const writeCypher = (cleanData) => {
            ...
            (variableName) => {
                ...
                placeRelationsCypher[variableName] = cleanData[variableName].map(
                    (place) => {
                        ...
                        const bodyFinal = body + `r.timeStamp = $timeStamp`
    ...
    const clearPlaceRelations = (cleanData) => {
        const result = `MATCH (:Person {userId:$userId})-[r]->(:CensusTract) DELETE r`
        return result
    }
...
export const readCypher = (cleanData) => {
    ...
    return {
        lastQuestionCypher: `MATCH (p:Person { userId :  $userId}) RETURN p.${config.systemGeneratedVariables.variableNameForLastQuestion} as ${config.systemGeneratedVariables.variableNameForLastQuestion}`
    }
}
